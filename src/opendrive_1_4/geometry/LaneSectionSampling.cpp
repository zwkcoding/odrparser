...
double laneSuperelevation(
    LateralProfileSuperelevationSet const &superelevationProfiles, double s)
{
    return evalPoly3(superelevationProfiles, s);
}

double laneShapeHeightEval(
...
std::pair<std::map<double, std::set<LateralProfileShape>>::const_iterator,
          std::map<double, std::set<LateralProfileShape>>::const_iterator>
laneShapeHeightBounds(
    std::map<double, std::set<LateralProfileShape>> const &shapeProfile,
    double s)
{
...
void normalizeEdge(Id const id, std::string const &what, Edge &edge)
{
    if (edge.size() <= 2u)
    {
        // if edge only consists of 2 points we could only remove the whole edge
        // in case the points are too near to each
        // other
        return;
    }
    ...
    for (std::size_t i = 1u; i < edge.size(); ++i)
    {
        ...
        if (i < edge.size() - 1)
        {
            ...
            if (from == to)
            {
                ...
                printf("normalizeEdge {%" PRIu64
                       "} dropping identical point from {%s} "
                       "edge at index {%" PRId64 "}\n",
                       id, what.c_str(), i);
                pointsToDrop++;
            }
            else
            {
                //  moving the middle line in a narrow curve often leads to
                //  artifacts (circles) which are removed by the below
                ...
                if (previousEdgeDir != Point(0., 0., 0.))
                {
                    ...
                    {
                        printf("normalizeEdge {%" PRIu64
                               "} extreme direction changing point "
                               "from {%s} edge at index {%" PRId64 "}\n",
                               id, what.c_str(), i);
                        pointsToDrop++;
                    }
                    else
    ...
    if (pointsToDrop > 0u)
    {
        ...
        printf("normalizeEdge {%" PRIu64 "} dropping points from {%s} edge "
               "{%" PRId64 "} remaining {%" PRId64 "}\n",
               id, what.c_str(), pointsToDrop, newEdgeSize);
...
void LaneSectionSampling::generateSamples()
{
    ...
    for (uint64_t i = 0u; i < roadInfo.lanes.lane_sections.size(); i++)
    {
        ...
        addSamples(roadInfo.lanes.lane_sections[i], laneSectionIndex,
                   samplingPoints);
    }
}

bool LaneSectionSampling::addSamples(
    LaneSection const &laneSection, uint64_t const &laneSectionIndex,
    std::list<double> const &laneSamplingPoints)
{
    ...
    {
        // the tolerance at the end of the lane sections is necessary so that
        // the lane offset applies only to the current
        // lane section
        s1 -= 1e-12;
    }
    ...
    if (s1 < s0)
    {
        printf("Invalid lane section length {%.3f} {%.3f}\n", s1, s0);
        ok = false;
    }
    ...
    std::copy_if(laneSamplingPoints.begin(), laneSamplingPoints.end(),
    ...
    {
        auto profile =
            evalProfile(laneSection, laneSectionIndex, localSamplingPoint);
    ...
    while (iter != profiles.end())
    {
        ...
        {
            // sampling below the Geometry::cMaxSamplingError longitudinal
            // distance makes no sense
            ++iter;
            continue;
        }
        auto const centerLinearInterpolated =
            interpolateProfile(*iter, *nextIter);
        auto centerActual = evalProfile(laneSection, laneSectionIndex,
...
LaneSectionSampling::evalProfile(LaneSection const &laneSection,
                                 ...
                                 double const &s)
{
    ...
    auto const laneShapeBounds =
        laneShapeHeightBounds(roadInfo.road_profiles.lateral_profile_shape, s);
    bool const laneShapeBoundsAvailable =
        (laneShapeBounds.first !=
             roadInfo.road_profiles.lateral_profile_shape.end() &&
         laneShapeBounds.second !=
             roadInfo.road_profiles.lateral_profile_shape.end());

    auto const angle = laneSuperelevation(
        roadInfo.road_profiles.lateral_profile_superelevation, s);
    auto const cos_angle = cos(angle);
    auto const sin_angle = sin(angle);

    double t = 0.;
    // store the shift of the outer superelevation operation to be applied to
    // lanes kept at level
    ...
    for (auto &laneInfo : laneSection.left)
    {
        ...
        // apply lane height
        auto laneHeightIter =
            laneInfo.lane_height.lower_bound(laneHeightSearch);
        ...
        if (laneInfo.attributes.level)
        {
            ...
            point.rightEdgePoint =
                point.rightEdgePoint + superelevationShiftOuterPoint;
            point.leftEdgePoint =
                point.leftEdgePoint + superelevationShiftOuterPoint;
        }
        else
        {
            if (laneShapeBoundsAvailable)
            {
                point.rightEdgePoint.z +=
                    laneShapeHeightEval(laneShapeBounds, s, rightT);
                point.leftEdgePoint.z +=
                    laneShapeHeightEval(laneShapeBounds, s, leftT);
            }

            point.rightEdgePoint =
                calculateSuperelevatedPoint(point.rightEdgePoint, rightT,
                                            centerPoint, cos_angle, sin_angle);
            auto const outerPoint = calculateSuperelevatedPoint(
                point.leftEdgePoint, leftT, centerPoint, cos_angle, sin_angle);
            superelevationShiftOuterPoint = outerPoint - point.leftEdgePoint;
            point.leftEdgePoint = outerPoint;
        }
        auto const id = laneId(roadInfo.attributes.id, laneSectionIndex,
    ...
    for (auto &laneInfo : laneSection.right)
    {
        ...
        else
        {
            ...
            point.leftEdgePoint = calculateSuperelevatedPoint(
                point.leftEdgePoint, leftT, centerPoint, cos_angle, sin_angle);
...
Point LaneSectionSampling::calculateSuperelevatedPoint(
    Point const &point, double t, DirectedPoint const &centerPoint,
    double const cos_angle, double const sin_angle)
{
    Point resultPoint;
    resultPoint.x =
        centerPoint.location.x + cos_angle * (point.x - centerPoint.location.x);
    resultPoint.y =
        centerPoint.location.y + cos_angle * (point.y - centerPoint.location.y);
    resultPoint.z = point.z + sin_angle * t;
    return resultPoint;
}

LaneSectionSampling::LaneSectionProfile
LaneSectionSampling::interpolateProfile(LaneSectionProfile const &start,
                                        LaneSectionProfile const &end)
{
    ...
    for (auto const &startLaneBorderPoint : start.laneBorderPoints)
    {
        ...
        if (findEnd != end.laneBorderPoints.end())
        {
            LaneBorderPoint point;
            point.rightEdgePoint =
                0.5 * (startLaneBorderPoint.second.rightEdgePoint +
                       findEnd->second.rightEdgePoint);
            point.leftEdgePoint =
                0.5 * (startLaneBorderPoint.second.leftEdgePoint +
                       findEnd->second.leftEdgePoint);
...
{
    auto const maxErrorSquared =
        Geometry::cMaxSamplingError * Geometry::cMaxSamplingError;
    ...
    while (iter != laneBorderPoints.end())
    {
        ...
        {
            auto errorSquaredLeft =
                (iter->second.leftEdgePoint - findOther->second.leftEdgePoint)
                    .normSquared();
            auto errorSquaredRight =
                (iter->second.rightEdgePoint - findOther->second.rightEdgePoint)
                    .normSquared();
            if ((errorSquaredLeft >= maxErrorSquared) ||
                (errorSquaredRight >= maxErrorSquared))
            {
        ...
        // the lane was already identified as acceptable before and is therefore
        // not present anymore in the interpolated
        // other
...
void LaneSectionSampling::writeLaneMap(LaneMap &laneMap)
{
    ...
    for (auto const &profile : profiles)
    {
        ...
        {
            laneMap[laneBorderPoint.first].leftEdge.push_back(
                laneBorderPoint.second.leftEdgePoint);
            laneMap[laneBorderPoint.first].rightEdge.push_back(
                laneBorderPoint.second.rightEdgePoint);
