...
BoostPolygon fromLane(Lane const &lane, double const margin)
{
    ...
    if (leftPoints.size() > 1)
    {
        for (; i < leftPoints.size() - 1; i++)
        {
            ...
            double normalizingFactor =
                1.0 /
                boost::geometry::distance(leftPoints[i + 1], leftPoints[i]);
            Boost2dPoint normalizedDirection = direction;
            boost::geometry::multiply_value(normalizedDirection,
                                            normalizingFactor);
    ...
    if (rightPoints.size() > 1)
    {
        ...
        for (; i < rightPoints.size() - 1; i++)
        {
            ...
            double normalizingFactor =
                1.0 /
                boost::geometry::distance(rightPoints[i + 1], rightPoints[i]);
            Boost2dPoint normalizedDirection = direction;
            boost::geometry::multiply_value(normalizedDirection,
                                            normalizingFactor);
...
void invertLaneAndNeighbors(LaneMap &laneMap, Lane &lane)
{
    ...
    while (leftNeighbor != 0u)
    {
        ...
        {
            printf("invertLaneAndNeighbors( {%" PRIu64 "} ) recursion\n",
                   lane.id);
            leftNeighbor = 0u;
        }
    }
...
void checkAddPredecessor(Lane &lane, Lane const &otherLane)
{
    ...
    {
        printf("checkAddPredecessor[ {%" PRIu64
               "} ] rejecting other lane: {%" PRIu64 "}\n",
               lane.id, otherLane.id);
    }
}
...
void checkAddSuccessor(Lane &lane, Lane const &otherLane)
{
    ...
    {
        printf("checkAddSuccessor[ {%" PRIu64
               "} ] rejecting other lane: {%" PRIu64 "}\n",
               lane.id, otherLane.id);
    }
}
...
Id laneId(int roadId, int laneSectionIndex, int laneIndex)
{
    ...
    {
        printf("Invalid lane section index {%d}\n", laneSectionIndex);
    }
                  ...
                  double const overlapMargin)
{
    ...
    try
    {
        ...
        (void)::boost::geometry::intersection(leftPolygon, rightPolygon,
                                              output);
        return (!output.empty());
    }
    catch (...)
    {
        printf("Error while checking overlap between lanes {%" PRIu64
               "} and {%" PRIu64 "}\n",
               leftLane.id, rightLane.id);
        return false;
    }
}
} // namespace geometry
