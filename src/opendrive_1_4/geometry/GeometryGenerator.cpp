...
#include "uos_odrparser/opendrive_1_4/geometry/CenterLine.hpp"
#include "uos_odrparser/opendrive_1_4/geometry/LaneSectionSampling.hpp"
#include "uos_odrparser/opendrive_1_4/geometry/LaneUtils.hpp"
#include <algorithm>
...
bool isInvalidLaneSection(::opendrive_1_4::LaneSection const &laneSection)
{
    auto length = laneSection.end_position - laneSection.start_position;
    if (length < MinimumSegmentLength)
    {
        printf("Invalid lane section of length {%.3f}\n", length);
        return true;
    }
    return false;
}
...
bool fixInvalidLaneSections(
    std::vector<::opendrive_1_4::LaneSection> &laneSections)
{
    for (auto it = laneSections.begin(); it != laneSections.end(); it++)
    {
...
void sortLanesByIndex(std::vector<::opendrive_1_4::LaneSection> &laneSections)
{
    auto compare = [](::opendrive_1_4::LaneInfo const &infoA,
                      ::opendrive_1_4::LaneInfo const &infoB) {
...
void generateTrafficSignal(TrafficSignalInformation &trafficSignalInfo,
                           ...
                           ::opendrive_1_4::LandmarkMap &landmarks)
{
    Landmark landmark;
    auto directedPoint =
        centerLine.eval(trafficSignalInfo.start_position, false);
    ...
    RoadInformation &roadInfo, LaneMap &laneMap)
{
    ...
    {
        printf("addTrafficReferenceToLanes() traffic reference outside road\n");
        return;
    }

    auto laneSectionIndex =
        static_cast<uint64_t>(laneSectionIt -
                              roadInfo.lanes.lane_sections.begin()) +
        1;
    double s = parametricPosition(trafficSignalReference.start_position,
                                  *laneSectionIt);
    for (auto &laneInfo : laneSectionIt->right)
    {
        if (trafficSignalReference.validityInfo.IsWithinRange(
...
void addTrafficSignals(RoadInformation &roadInfo, CenterLine const &centerLine,
                       opendrive_1_4::OpenDriveData &mapData)
{
    for (auto &trafficSignalInfo : roadInfo.traffic_signals)
    {
        ...
        trafficSignalReference.start_position =
            trafficSignalInfo.start_position;
        trafficSignalReference.validityInfo.from_lane =
            trafficSignalInfo.validityInformation.from_lane;
        trafficSignalReference.validityInfo.to_lane =
            trafficSignalInfo.validityInformation.to_lane;
             ...
             LaneInfo const &laneInfo, Id laneId)
{
    ...
    {
        printf("Duplicated lane Id %" PRIu64 "\n", laneId);
        return false;
    }
    ...
    {
        mapData.intersectionLaneIds[roadInfo.attributes.junction].push_back(
            laneId);
    }
...
bool initializeLaneMap(opendrive_1_4::OpenDriveData &mapData)
{
    bool ok = true;
...
double convertToMetersPerSecond(double const value, std::string const &units)
{
    ...
    {
        printf("Unrecognized speed units\n");
        return 0.0;
    }
}
...
double speedAt(double s, std::vector<RoadSpeed> speed)
{
    ...
    printf("speedAt() Invalid parameter {%.3f}\n", s);
                                             ...
                                             std::vector<RoadSpeed> speed)
{
    ...
    if (length <= 0)
    {
        printf("parametricSpeed() Invalid parameters: {%.3f} <= {%.3f}\n", s1,
               s0);
    ...
    {
        printf("parametricSpeed() road segment too short length = {%.3f}\n",
               length);
        return {ParametricSpeed(speedAt(s0, speed))};
    }
    ...
    {
        ::opendrive_1_4::ParametricSpeed parametricSpeed;
                                                ...
                                                double laneSectionLength)
{
    ...
    {
        printf("calculateLaneSpeed:: lane section = {%.3f} length too short\n",
               laneSectionLength);
    }
         ...
         ++it)
    {
        ...
        parametricSpeed.speed =
            convertToMetersPerSecond(it->max_speed, it->unit);
...
void calculateSpeed(RoadInformation &roadInfo, LaneMap &laneMap)
{
    ...
    for (auto &laneSection : laneSections)
    {
        ...
        {
            laneSectionSpeed =
                parametricSpeed(laneSection.start_position,
...
bool checkId(Id id, std::string const &text = "")
{
    if (id < 1)
    {
        printf("checkId() invalid id {%" PRIu64 "} {%s}\n", id, text.c_str());
        return false;
    }
    return true;
}

bool belongsToIntersection(::opendrive_1_4::RoadInformation const &roadInfo)
{
    return roadInfo.attributes.junction == -1 ? false : true;
}

void setPredecessor(::opendrive_1_4::OpenDriveData &mapData,
                    ::opendrive_1_4::RoadInformation const &roadInfo,
                    ::opendrive_1_4::LaneInfo const &laneInfo)
{
    ...
    if (roadIt != mapData.roads.end())
    {
        ...
        Id predecessorId{0u};
        if (contact == ::opendrive_1_4::ContactPoint::End)
        {
        ...
        else if (contact == ::opendrive_1_4::ContactPoint::Start)
        {
...
void setSuccessor(::opendrive_1_4::OpenDriveData &mapData,
                  ::opendrive_1_4::RoadInformation const &roadInfo,
                  ::opendrive_1_4::LaneInfo const &laneInfo)
{
...
bool hasSuccessorRoad(RoadInformation const &roadInfo)
{
    ...
    return link.successor != nullptr &&
           link.successor->element_type == ::opendrive_1_4::ElementType::Road;
}
...
bool hasPredecessorRoad(RoadInformation const &roadInfo)
{
    ...
    return link.predecessor != nullptr &&
           link.predecessor->element_type == ::opendrive_1_4::ElementType::Road;
}

void setSuccessorPredecessor(::opendrive_1_4::OpenDriveData &mapData,
                             ::opendrive_1_4::RoadInformation const &roadInfo,
                             ::opendrive_1_4::LaneInfo const &laneInfo,
                             uint64_t const laneSectionIndex)
{
    ...
    Id currentLaneId = laneId(roadInfo.attributes.id, laneSectionIndex,
                              laneInfo.attributes.id);
    auto &laneSections = roadInfo.lanes.lane_sections;
    ...
    else
    {
        ...
        {
            Id predecessorId =
                laneId(roadInfo.attributes.id, laneSectionIndex - 1,
                       laneInfo.link->predecessor_id);
            ...
            {
                printf("Warning: predecessorId for road  {%d} lane {%d} and "
                       "section {%" PRIu64 "} does not exist",
                       roadInfo.attributes.id, laneInfo.link->predecessor_id,
                       laneSectionIndex - 1);
            }
        }
    }
...
void setLeftRightNeighbor(::opendrive_1_4::OpenDriveData &mapData,
                          ::opendrive_1_4::RoadInformation const &roadInfo,
                          uint64_t const laneSectionIndex)
{
    ...
    for (std::size_t k = 0; k < laneSection.left.size(); k++)
    {
        ...
        if (k == 0)
        {
            ...
            {
                rightNeighborId =
                    laneId(roadInfo.attributes.id, laneSectionIndex,
                      ...
                      opendrive_1_4::OpenDriveData &mapData)
{
    ...
    for (std::size_t i = 0; i < laneSections.size(); ++i)
    {
        ...
        {
            auto &laneInfo = laneSection.left[k];
            setSuccessorPredecessor(mapData, roadInfo, laneInfo,
                                    laneSectionIndex);
        }
        for (std::size_t k = 0; k < laneSection.right.size(); k++)
        {
...
void autoConnectIntersectionLanes(opendrive_1_4::OpenDriveData &mapData,
                                  double const overlapMargin)
{
    ...
    for (auto &element : intersectionLaneIds)
    {
        ...
        for (auto leftIt = lanes.begin(); leftIt != lanes.end(); leftIt++)
        {
            ...
            {
                if (lanesOverlap(laneMap[*leftIt], laneMap[*rightIt],
                                 overlapMargin))
                {
                ...
                switch (contactPlace(laneMap[*leftIt], laneMap[*rightIt]))
                {
                    case ContactPlace::LeftLeft:
                        ...
                        invertLaneAndNeighbors(laneMap,
                                               mapData.laneMap[*rightIt]);
                    // fall through
...
double length(::opendrive_1_4::Edge const &edge)
{
...
void fixNeighborEdgesAtEnd(opendrive_1_4::OpenDriveData &mapData,
                           Lane const &lane)
{
...
void fixNeighborEdgesAtBegin(opendrive_1_4::OpenDriveData &mapData,
                             Lane const &lane)
{
    if (lane.leftNeighbor != 0)
    {
...
bool checkLaneConsistency(opendrive_1_4::OpenDriveData &mapData)
{
    bool ok = true;
    auto &laneMap = mapData.laneMap;
    ...
    for (auto &element : laneMap)
    {
        ...
        {
            printf("checkLaneConsistency:: Invalid lane geometry for lane "
                   "{%" PRIu64 "}\n",
                   lane.id);
        ...
        // if the points don't match exactly, the lanes either overlap a bit or
        // we have a gap
        // this is due to inaccuracies on geometry generation and the near()
        // function for topology creation
        // allowing bigger differences (otherwise the topology would be broken
        // and routing would fail)
        // to fix this, we either shorten a bit or extend to fill the gap (delta
        // of the gap is < 1cm)
        // adapting lanes with multiple successors/predecessors is not a good
        // option, because
        // fixing the connection to one might affect the connections to others
        // therefore, auto-fixing is only done for now, if one of the two has
        // only a single connection
        // in addition, the fix has to be applied to the respective left/right
        // neighbor since they
        ...
        for (auto successorId : lane.successors)
        {
            ...
            if (contains(successorLane.predecessors, lane.id))
            {
                    ...
                    (lane.rightEdge.back() != successorLane.rightEdge.front()))
                {
                    ...
                    {
                        printf("checkLaneConsistency:: Auto-fix of lane end "
                               "points not "
                               "possible for lanes {%" PRIu64 "} -> {%" PRIu64
                               "}\n",
                               lane.id, successorId);
                        ok = false;
                    }
            ...
            {
                printf("checkLaneConsistency:: Invalid lane topology for lane "
                       "{%" PRIu64 "}\n",
                       lane.id);
            }
        }
        for (auto predecessorId : lane.predecessors)
        {
            ...
            {
                if ((lane.leftEdge.front() !=
                     predecessorLane.leftEdge.back()) ||
                    (lane.rightEdge.front() !=
                     predecessorLane.rightEdge.back()))
                {
                    ...
                    {
                        printf("checkLaneConsistency:: Auto-fix of lane start "
                               "points "
                               "not possible for lanes {%" PRIu64
                               "} <- {%" PRIu64 "}\n",
                               lane.id, predecessorId);
                        ok = false;
                    }
                }
            }
            ...
            {
                if ((lane.leftEdge.front() !=
                     predecessorLane.rightEdge.front()) ||
                    (lane.rightEdge.front() !=
                     predecessorLane.leftEdge.front()))
                {
                    if (lane.predecessors.size() == 1u)
                    {
                        lane.leftEdge.front() =
...
#if 0
bool convertToGeoPoints(opendrive_1_4::OpenDriveData &mapData)
{
    ...
    if (projPtr == nullptr)
    {
        ...
        printf("Using default projection: {%s}\n", defaultProjection.c_str());
        if (projPtr == nullptr)
        {
            printf("Unknown error while creating the projection\n");
            return false;
        }
    }
    ...
    for (auto &element : mapData.laneMap)
    {
        ...
        {
            printf("convertToGeoPoints>> error converting ENU points of lane "
                   "{%" PRIu64 "} "
                   "to Geo Points\n",
                   lane.id);
