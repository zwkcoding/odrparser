                                         ...
                                         bool applyLateralOffset) const
{
    if (s < 0.)
    {
        // as the curve is only defined betwen the range [0 , length], we return
        // the line evaluated at s = 0
        return eval(0.);
    }
    ...
    for (auto it = geometry.rbegin(); it != geometry.rend(); it++)
    {
        ...
        {
            auto directedPoint =
                (*it)->PosFromDist(s - (*it)->GetStartOffset());
    ...
    auto directed_point =
        lastGeometry->PosFromDist(s - lastGeometry->GetStartOffset());
...
std::list<double> CenterLine::samplingPoints() const
{
    ...
    while (iter != points.end())
    {
        ...
        auto errorSquared =
            (centerActual - centerLinearInterpolated).normSquared();
...
bool generateCenterLine(RoadInformation &roadInfo, CenterLine &centerLine)
{
    ...
    for (auto &geometry_attribute : roadInfo.geometry_attributes)
    {
        Point start(geometry_attribute->start_position_x,
                    ...
                    geometry_attribute->start_position_z);
            ...
            switch (geometry_attribute->type)
            {
                case ::opendrive::GeometryType::ARC:
                {
                    ...
                    centerLine.geometry.emplace_back(
                        std::make_unique<geometry::GeometryArc>(
                            arc->start_position, arc->length, arc->heading,
                            start, arc->curvature));
                    break;
                }
                case ::opendrive::GeometryType::LINE:
                {
                    ...
                    centerLine.geometry.emplace_back(
                        std::make_unique<geometry::GeometryLine>(
                            line->start_position, line->length, line->heading,
                            start));
                    break;
                }
                break;
                ...
                case ::opendrive::GeometryType::POLY3:
                {
                    ...
                    centerLine.geometry.emplace_back(
                        std::make_unique<geometry::GeometryPoly3>(
                            poly3->start_position, poly3->length,
                            poly3->heading, start, poly3->a, poly3->b, poly3->c,
                            poly3->d));
                    break;
                }
                break;
                case ::opendrive::GeometryType::PARAMPOLY3:
                {
                    ...
                    centerLine.geometry.emplace_back(
                        std::make_unique<geometry::GeometryParamPoly3>(
                            ...
                            paramPoly3->heading, start, paramPoly3->aU,
                            paramPoly3->bU, paramPoly3->cU, paramPoly3->dU,
                            paramPoly3->aV, paramPoly3->bV, paramPoly3->cV,
                            paramPoly3->dV,
        ...
        {
            printf("generateCenterLine() Invalid geometry definition");
            ok = false;
        }
    }
