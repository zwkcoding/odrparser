        ...
        for (pugi::xml_node node_object : node_objects.children("object"))
        {
            ...
            if (gRoadObjects.type == "crosswalk")
            {
                ...
                if (node_outline)
                {
                    ...
                    {
                        gRoadObjects.outline.u =
                            node_corner.attribute("u").as_double();
                        gRoadObjects.outline.v =
                            node_corner.attribute("v").as_double();
                        gRoadObjects.outline.z =
                            node_corner.attribute("z").as_double();
                    }
                }
            }
            else if (gRoadObjects.name.substr(0, 6) == "Speed_" ||
                     gRoadObjects.name.substr(0, 6) == "speed_")
            {
                std::string speed_str = gRoadObjects.name.substr(6);
                gRoadObjects.speed = std::stod(speed_str);
            }
            else if (gRoadObjects.name.find("Stencil_STOP") !=
                     std::string::npos)
            {
            }
