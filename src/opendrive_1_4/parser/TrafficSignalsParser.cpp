    ...
    const pugi::xml_node &parent_node, const std::string &node_name)
{
    ...
    if (validityNode)
    {
        validityInfo.from_lane =
            std::stoi(validityNode.attribute("fromLane").value());
        validityInfo.to_lane =
            std::stoi(validityNode.attribute("toLane").value());
    }
    return validityInfo;
}
        ...
        &out_traffic_signal_references)
{
         ...
         signal = signal.next_sibling("signal"))
    {
        ...
        {
            trafficSignalInformation.height =
                assignDefaultValues(signal.attribute("height").value(), 0.0);
        }
        if (signal.attribute("width") != nullptr)
        {
            trafficSignalInformation.width =
                assignDefaultValues(signal.attribute("width").value(), 0.0);
        }
        ...
        {
            trafficSignalInformation.hoffset =
                assignDefaultValues(signal.attribute("hOffset").value(), 0.0);
        }
        if (signal.attribute("pitch") != nullptr)
        {
            trafficSignalInformation.pitch = assignDefaultValues(
                signal.attribute("pitch").value(), 2 * M_PI);
        }
        if (signal.attribute("roll") != nullptr)
        {
            trafficSignalInformation.roll =
                assignDefaultValues(signal.attribute("roll").value(), 2 * M_PI);
        }

        trafficSignalInformation.validityInformation =
            AddValidity(signal, "validity");
        ...
        for (pugi::xml_node position_node_inertial :
             signal.children("positionInertial"))
        {
            ...
            trafficSignalInformation.physical_position_inertial.hdg =
                position_node_inertial.attribute("hdg").as_double();
            trafficSignalInformation.physical_position_inertial.pitch =
                position_node_inertial.attribute("pitch").as_double();
            trafficSignalInformation.physical_position_inertial.roll =
                position_node_inertial.attribute("roll").as_double();
        }

        for (pugi::xml_node position_node_road :
             signal.children("positionRoad"))
        {
            ...
            trafficSignalInformation.physical_position_road.sOffset =
                position_node_road.attribute("s").as_double();
            trafficSignalInformation.physical_position_road.tOffset =
                position_node_road.attribute("t").as_double();
            trafficSignalInformation.physical_position_road.zOffset =
                position_node_road.attribute("zOffset").as_double();
            trafficSignalInformation.physical_position_road.hOffset =
                position_node_road.attribute("hOffset").as_double();
            trafficSignalInformation.physical_position_road.pitch =
                position_node_road.attribute("pitch").as_double();
            trafficSignalInformation.physical_position_road.roll =
                position_node_road.attribute("roll").as_double();
        }
         ...
         signal = signal.next_sibling("signalReference"))
    {
        ...
        for (pugi::xml_node userData =
                 xmlNode.child("signalReference").child("userData");
             userData; userData = userData.next_sibling("userData"))
        {
            std::string node_name = "vectorSignal";
            for (pugi::xml_node vector_node = userData.child(node_name.c_str());
                 vector_node;
                 vector_node = vector_node.next_sibling("vectorSignal"))
            {
                trafficSignalReference.relation =
                    vector_node.attribute("turnRelation").value();
            }
        }
