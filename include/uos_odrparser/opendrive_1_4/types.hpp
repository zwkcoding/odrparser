...
struct Point
{
    ...
    bool operator==(const Point &rhs) const
    {
        ...
        return (std::fabs(x - rhs.x) < cCoordinateTolerance) &&
               (std::fabs(y - rhs.y) < cCoordinateTolerance) &&
    ...
    void ensureValid() const
    {
        if (!isValid())
        {
            printf("ensureValid(Point)>> {%.3f, %.3f, %.3f} value out of range",
                   x, y, z);
...
struct ParametricSpeed
{
    ...
    explicit ParametricSpeed(double _speed) : start{0.}, end{1.0}, speed{_speed}
    {
    }
};
...
{
    double s_position{0.}; // Shape polynomial is in t-axis direction, a set of
                           // this is placed at different s-positions
};
...
struct TrafficSignPositionInertial
{
    ...
    double hdg{
        0.}; // rad: Heading of the signal, relative to the inertial system
    double pitch{0.}; // rad: Pitch angle of the signal after applying heading,
                      // relative to the inertial system (x’y’-plane)
    double roll{0.}; // rad: Roll angle of the signal after applying heading and
                     // pitch, relative to the inertial system
                     // (x’’y’’-plane)
};
...
struct TrafficSignPositionRoad
{
    ...
    double zOffset{
        0.}; // m: z offset from road level to bottom edge of the signal
    double hOffset{
        0.}; // rad: Heading offset of the signal (relative to @orientation)
    double pitch{0.}; // rad: Pitch angle of the signal after applying hOffset,
                      // relative to the inertial system (x’y’-plane)
    double roll{0.}; // rad: Roll angle of the signal after applying hOffset and
                     // pitch, relative to the inertial system
                     // (x’’y’’-plane)
};
