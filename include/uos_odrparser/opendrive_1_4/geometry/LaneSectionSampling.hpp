...
{
public:
    LaneSectionSampling(RoadInformation const &roadInfo_,
                        CenterLine const &centerLine_)
        : roadInfo(roadInfo_), centerLine(centerLine_)
    {
    }
...
private:
    ...
    Point calculateSuperelevatedPoint(Point const &point, double t,
                                      DirectedPoint const &centerPoint,
                                      double const cos_angle,
                                      double const sin_angle);
    LaneSectionProfile evalProfile(LaneSection const &laneSection,
                                   uint64_t const &laneSectionIndex,
                                   double const &s);
    bool addSamples(LaneSection const &laneSection,
                    uint64_t const &laneSectionIndex,
                    std::list<double> const &laneSamplingPoints);
    LaneSectionProfile interpolateProfile(LaneSectionProfile const &start,
