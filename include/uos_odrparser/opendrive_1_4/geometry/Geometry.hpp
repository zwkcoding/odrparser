...
/**
 * @brief DirectedPoint is composed of a 3 dimensional position and an
 * orientation
 */
struct DirectedPoint
{
    ...
    /**
     * @brief Shifts the point by lateral_offset meters in the normal direction
     * of the tangent
     */
    void ApplyLateralOffset(double lateral_offset);

    /**
     * @returns the point shifted by lateral_offset meters in the normal
     * direction of the tangent
     */
    Point getLateralOffsetPoint(double lateral_offset) const;

    /**
     * @brief Compares two points, returns true if their parameters are
     * identical
     */
...
public:
    GeometryPoly3(double start_offset, double length, double heading,
                  const Point &start_pos, double a, double b, double c,
                  double d);
...
template <class T>
double evalPoly3(std::set<T> const &sOffsetPoly3Set, double s)
{
    T search;
    ...
    {
        auto poly =
            boost::array<double, 4>{{smallerOrEqual->a, smallerOrEqual->b,
                                     smallerOrEqual->c, smallerOrEqual->d}};
        return boost::math::tools::evaluate_polynomial(
            poly, s - smallerOrEqual->start_offset);
    }
    return 0.0;
}
